...
void RangeSelectorTableDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
    ...
    painter->save();
    // Using a try-catch block is a robust way to ensure painter->restore() is always called,
    // even if an exception is thrown during painting (e.g., from the model).
    try {
        auto options = option;
        ...
        float fold_prob = 1.0f - range_float;
        int disable_height = static_cast<int>(fold_prob * static_cast<float>(option.rect.height()));
