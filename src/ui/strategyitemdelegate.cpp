...
void StrategyItemDelegate::paint_strategy(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index, bool withEVs) const {
    ...
    shared_ptr<GameTreeNode> node = tableStrategyModel->getCurrentNode().lock();
    if (node && node->getType() == GameTreeNode::GameTreeNode::ACTION) {
        vector<pair<GameActions,float>> strategy = tableStrategyModel->get_strategy(index.row(),index.column());
            ...
            if(!strategy.empty()){
                ...
                float not_in_range = (tableStrategyModel->get_detail_window_setting()->grid_i == index.row()) &&
                                       (tableStrategyModel->get_detail_window_setting()->grid_j == index.column())
                                        ? 0 : 1 - range_number;
...
void StrategyItemDelegate::paint_range(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
    ...
    if (tableStrategyModel == nullptr) {
        return;
    }

    vector<pair<int,int>> card_cords;
    if (static_cast<size_t>(index.row()) >= tableStrategyModel->ui_p1_range.size() ||
        static_cast<size_t>(index.row()) >= tableStrategyModel->ui_p2_range.size() ||
        static_cast<size_t>(index.column()) >= tableStrategyModel->ui_p1_range[index.row()].size() ||
        static_cast<size_t>(index.column()) >= tableStrategyModel->ui_p2_range[index.row()].size()) {
        return;
    }

    if(tableStrategyModel->get_detail_window_setting()->mode == DetailWindowSetting::DetailWindowMode::RANGE_IP){
...
void StrategyItemDelegate::paint_evs(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
    ...
    initStyleOption(&options, index);
    const TableStrategyModel * tableStrategyModel = qobject_cast<const TableStrategyModel*>(index.model());
    if (!tableStrategyModel || !tableStrategyModel->get_qsolverjob()) {
        return;
    }
    ...
    for(std::size_t i = 0;i < evs.size();i ++ ){
        ...
        float normalized_ev = normalization_tanh(tableStrategyModel->get_qsolverjob()->stack,one_ev);
...
void StrategyItemDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
    ...
    const TableStrategyModel * tableStrategyModel = qobject_cast<const TableStrategyModel*>(index.model());
    if (!tableStrategyModel || !tableStrategyModel->get_detail_window_setting()) {
        painter->restore();
        return;
    }

    if(tableStrategyModel->get_detail_window_setting()->mode == DetailWindowSetting::DetailWindowMode::STRATEGY){
        this->paint_strategy(painter,option,index);
    }
    else if(tableStrategyModel->get_detail_window_setting()->mode == DetailWindowSetting::DetailWindowMode::RANGE_IP ||
            tableStrategyModel->get_detail_window_setting()->mode == DetailWindowSetting::DetailWindowMode::RANGE_OOP ){
        this->paint_range(painter,option,index);
    }
    else if(tableStrategyModel->get_detail_window_setting()->mode == DetailWindowSetting::DetailWindowMode::EV){
        this->paint_strategy(painter,option,index,true);
    }
    else if(tableStrategyModel->get_detail_window_setting()->mode == DetailWindowSetting::DetailWindowMode::EV_ONLY){
